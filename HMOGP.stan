...
parameters {
  array[N_visits] vector[N_dimensions] mu_common;
  array[N_visits] vector<lower=0>[N_dimensions] sigma_patient;
  array[N_visits] matrix[N_patients, N_dimensions] mu_patient;
  array[N_visits] matrix[N_times, N_dimensions] eta_common;
  array[N_visits, N_patients] matrix[N_times, N_dimensions] eta_patient;

  array[N_visits] vector<lower=0>[N_dimensions] alpha_common; // common/shared temporal pattern: kernel scale (sqrt of kernel variance)
  ...
  array[N_visits] cholesky_factor_corr[N_dimensions] L_Omega_common;
...
transformed parameters {
  array[N_visits] matrix[N_times, N_dimensions] f_common; // convert it to vector with N_times
  {
    ...
    for (v in 1:N_visits) {
      f_common[v] = L_common * eta_common[v] * diag_pre_multiply(alpha_common[v], L_Omega_common[v])';
    }
  }
      ...
      for (t in 1:N_times) {
        for (d in 1:N_dimensions) {
          f[v, p, t, d] = mu_patient[v, p, d] // patient's deviation in baseline
                          + f_common[v, t, d] //f_common_response
...
model {
  for (v in 1:N_visits) {
    mu_common[v] ~ normal(0, 1);
    sigma_patient[v] ~ inv_gamma(1, 0.5);
    for (d in 1:N_dimensions) {
      mu_patient[v, , d] ~ normal(mu_common[v, d], sigma_patient[v, d]);
    }
    to_vector(eta_common[v]) ~ normal(0, 1);
    L_Omega_common[v] ~ lkj_corr_cholesky(1);
    alpha_common[v] ~ inv_gamma(1, 1);
  }
        ...
        for (d in 1:N_dimensions) {
          if (mask[v, p, t, d] == 1) {
            if (t == 1) {
              ...
              0 ~ normal(f_common[v, t, d], sqrt(pow(sigma[d], 2) * 0.495));
...
generated quantities {
  array[N_visits] matrix[N_dimensions, N_dimensions] Omega_common;
  array[N_visits, N_patients] matrix[N_dimensions, N_dimensions] Omega_patient;
  array[N_visits] matrix[N_times_pred, N_dimensions] f_common_pred;
  array[N_visits, N_patients] matrix[N_times_pred, N_dimensions] f_patient_pred;
  for (v in 1:N_visits) {
    Omega_common[v] = L_Omega_common[v] * L_Omega_common[v]';
    f_common_pred[v] = gp_pred_multi_rng(times_pred, times, f_common[v], lengthscale_common, alpha_common[v], L_Omega_common[v]);
    for (p in 1:N_patients) {
  ...
  for (v in 1:N_visits) {
    for (p in 1:N_patients) {
      for (t in 1:N_times_pred) {
        for (d in 1:N_dimensions) {
          f_pred[v, p, t, d] = mu_patient[v, p, d] + f_common_pred[v, t, d] + f_patient_pred[v, p, t, d];
